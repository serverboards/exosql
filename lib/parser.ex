...
defmodule ExoSQL.Parser do
  ...
  defp real_parse(parsed, context) do
    ...
    all_tables =
      if join != [] do
        from ++
          Enum.map(join, fn {_type, {table, _expr}} ->
    ...
    join =
      Enum.map(join, fn {type, {table, expr}} ->
        {type,
         {
           resolve_table(table, all_tables_at_context, context),
  ...
  def resolve_all_columns(tables, context) do
    all_columns =
      Enum.flat_map(tables, fn
        ...
        {:alias, {table, alias_}} ->
          resolve_all_columns([table], context)
          |> Enum.map(fn {_db, _table, column} -> {:tmp, alias_, column} end)
